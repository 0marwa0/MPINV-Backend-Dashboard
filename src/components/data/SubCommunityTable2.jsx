...
export default function SubCommunityTable2() {
    ...
    },
    {
      ...
      render: (row) => (
        <div className="flex items-center gap-1">
          <button type="button" title="Edit" className="text-gray-700 hover:text-black p-2" onClick={() => startEdit(row)}>
  ...
  const onDelete = (row) => { setRowToDelete(row); setConfirmOpen(true) }
  const confirmDelete = async () => {
    const row = rowToDelete; setConfirmOpen(false)
    if (!row || row.id == null) return
    try {
      const res = await fetch(`/api/sub-community/${encodeURIComponent(row.id)}`, { method: 'DELETE' })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
  ...
  const startEdit = (row) => {
    setEditRow(row)
    setEditForm({ name: row.name || '', community_id: String(row.community_id || '') })
    setEditOpen(true)
    setEditError(null)
  }
  ...
  const submitEdit = async (e) => {
    e.preventDefault()
    if (!editRow) return
    if (!editForm.name.trim() || !editForm.community_id) { setEditError('Name and community are required'); return }
    setEditSubmitting(true)
    try {
      const payload = { name: editForm.name.trim(), community_id: Number(editForm.community_id) }
      const res = await fetch(`/api/sub-community/${encodeURIComponent(editRow.id)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      })
  ...
  return (
    <div className="space-y-3">
          ...
          <div className="w-[92vw] max-w-md rounded-xl bg-white p-5 shadow-lg">
            ...
            <form onSubmit={onSubmit} className="space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" disabled={submitting} className="px-3 py-2 rounded-md bg-black text-white disabled:opacity-50">{submitting ? 'Saving…' : 'Save'}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-[92vw] max-w-md rounded-xl bg-white p-5 shadow-lg">
            ...
            <form onSubmit={submitEdit} className="space-y-3">
              <label className="block">
                <span className="block text-sm text-gray-600 mb-1">Name</span>
                <input value={editForm.name} onChange={(e)=>setEditForm({...editForm, name: e.target.value})} className="w-full rounded-md border border-black/20 px-3 py-2" />
              </label>
              <label className="block">
                <span className="block text-sm text-gray-600 mb-1">Community</span>
                <select value={editForm.community_id} onChange={(e)=>setEditForm({...editForm, community_id: e.target.value})} className="w-full rounded-md border border-black/20 px-3 py-2 bg-white">
                  <option value="" disabled>Select a community</option>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" disabled={editSubmitting} className="px-3 py-2 rounded-md bg-black text-white disabled:opacity-50">{editSubmitting ? 'Saving…' : 'Save'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <ConfirmDialog
        ...
        message={rowToDelete ? `Delete sub community "${rowToDelete.name}"? This action cannot be undone.` : ''}
        confirmText="Delete"
        cancelText="Cancel"
