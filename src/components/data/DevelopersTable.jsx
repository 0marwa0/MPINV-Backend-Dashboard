...
export default function DevelopersTable() {
  ...
  const [form, setForm] = useState({ name: "", logoFile: null });
  ...
  const loadDevelopers = async (p = page, l = limit) => {
    try {
      setLoading(true);
      setError(null);
      const params = new URLSearchParams({ page: String(p), limit: String(l) });
      const res = await fetch(`/api/developer?${params.toString()}`);
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/developer/${encodeURIComponent(row.id)}`, {
        method: "DELETE",
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  const onSubmitEdit = async (e) => {
    ...
    if (!editForm.name.trim()) {
      setEditError("Name and logo are required");
      return;
    }
    ...
    try {
      ...
      setEditOpen(false);
      setEditRow(null);
      await loadDevelopers(page, limit);
    } catch (e) {
      setEditError(e?.message || "Failed to update");
      toast.error(e?.message || "Failed to update Developer");
    } finally {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/developer", { method: "POST", body: fd });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const returned = await res.json().catch(() => null);
      // Map created item if backend returns it; otherwise use form values
      const created =
        returned && (returned.data || returned.developer || returned);
      ...
      toast.success("Dveloper created");
      setShowForm(false);
      setForm({ name: "", logoFile: null });
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between text-sm text-gray-600">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={page <= 1}
            onClick={() => loadDevelopers(page - 1, limit)}
          >
            Previous
          </button>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-[92vw] max-w-md rounded-xl bg-white p-5 shadow-lg">
            <h3 className="text-lg font-semibold mb-4">Create Developer</h3>
      ...
      <ConfirmDialog
        ...
        message={
          rowToDelete
            ? `Delete developer "${rowToDelete.name}"? This action cannot be undone.`
            : ""
        }
        confirmText="Delete"
        cancelText="Cancel"
      ...
      {editOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-[92vw] max-w-md rounded-xl bg-white p-5 shadow-lg">
            <h3 className="text-lg font-semibold mb-4">Edit Developer</h3>
