...
export default function CommunityTable() {
  ...
  const columns = [
    ...
    { key: 'actions', header: '', render: (row) => (
      <div className="flex items-center gap-1">
        <button type="button" title="Edit" className="text-gray-700 hover:text-black p-2" onClick={() => onStartEdit(row)}>
        ...
        <button type="button" title="Delete" className="text-red-600 hover:text-red-700 p-2" onClick={() => onDelete(row)}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
  ...
  const loadCommunities = async (p = page, l = limit) => {
    try {
      ...
      const newTotal = Number(any?.total ?? any?.count ?? total ?? mapped.length)
  ...
  const loadStates = async () => {
    try {
      ...
      const list = Array.isArray(any?.data)
        ...
        ? any.states
        : []
      const mapped = list.map((it) => ({ id: it.id ?? it._id ?? it.code ?? it.state_code, name: String(it.name ?? it.state_name ?? '') }))
  ...
  const [editForm, setEditForm] = useState({ name: '', state_id: '', logoFile: null })
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/community/${encodeURIComponent(row.id)}`, { method: 'DELETE' })
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
  ...
  const onStartEdit = (row) => {
    setEditRow(row)
    setEditForm({ name: row.name || '', state_id: String(row.state_id ?? ''), logoFile: null })
    setEditError(null)
    setEditOpen(true)
  }
  ...
  const onSubmitEdit = async (e) => {
    ...
    try {
      ...
      if (editForm.logoFile) {
        ...
        res = await fetch(`/api/community/${encodeURIComponent(editRow.id)}`, { method: 'PUT', body: fd })
      } else {
        res = await fetch(`/api/community/${encodeURIComponent(editRow.id)}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: editForm.name.trim(), state_id: Number(editForm.state_id) })
        })
      }
      if (!res.ok) throw new Error(`HTTP ${res.status}`)
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const created = returned && (returned.data || returned.community || returned)
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <label className="text-sm text-gray-600 hidden md:flex items-center gap-2">
            ...
            <select
              value={limit}
              onChange={(e) => { const v = Number(e.target.value) || 10; setLimit(v); loadCommunities(1, v) }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-[92vw] max-w-md rounded-xl bg-white p-5 shadow-lg">
            ...
            <form onSubmit={onSubmit} className="space-y-3">
              ...
              <label className="block">
                <span className="block text-sm text-gray-600 mb-1">Logo File</span>
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => setForm({ ...form, logoFile: e.target.files && e.target.files[0] ? e.target.files[0] : null })}
                  className="w-full rounded-md border border-black/20 px-3 py-2 bg-white"
                />
                {form.logoFile && (
                  <p className="mt-1 text-xs text-gray-500">Selected: {form.logoFile.name}</p>
                )}
              </label>
              <label className="block">
                ...
                <select
                  ...
                  className="w-full rounded-md border border-black/20 px-3 py-2 bg-white"
                >
                  <option value="" disabled>{statesLoading ? 'Loading states…' : 'Select a state'}</option>
                  {!statesLoading && states.map((s) => (
      ...
      <ConfirmDialog
        ...
        message={rowToDelete ? `Delete community "${rowToDelete.name}"? This action cannot be undone.` : ''}
        confirmText="Delete"
        cancelText="Cancel"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-[92vw] max-w-md rounded-xl bg-white p-5 shadow-lg">
            ...
            <form onSubmit={onSubmitEdit} className="space-y-3">
              ...
              <label className="block">
                <span className="block text-sm text-gray-600 mb-1">Replace Logo (optional)</span>
                <input type="file" accept="image/*" className="w-full rounded-md border border-black/20 px-3 py-2 bg-white"
                  onChange={(e) => setEditForm({ ...editForm, logoFile: e.target.files && e.target.files[0] ? e.target.files[0] : null })}
                />
              </label>
              {editError && <p className="text-sm text-red-600">{editError}</p>}
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" className="px-3 py-2 rounded-md border border-black/20" onClick={() => { setEditOpen(false); setEditRow(null); setEditError(null) }}>Cancel</button>
                <button type="submit" disabled={editSubmitting} className="px-3 py-2 rounded-md bg-black text-white disabled:opacity-50">
